...
var _ = Describe("Scenario: High Volume Reads", func() {
	...
	Measure("should result in measurements of p99, p50 and avg for all successful read requests to elasticsearch", func(b Benchmarker) {
		...
		Expect(err).Should(Succeed(), "Failed to read QPS for all elasticsearch reads with status code 2xx")
		b.RecordValue("All elasticsearch 2xx reads QPS", qps)

		// Record latency p99
		p99, err := metricsClient.RequestDurationOkSearchP99(job, defaultRange)
		Expect(err).Should(Succeed(), "Failed to read p50 for all elasticsearch reads with status code 2xx")
		b.RecordValue("All elasticsearch 2xx reads p99", p99)

		// Record latency p50
		p50, err := metricsClient.RequestDurationOkSearchP50(job, defaultRange)
		Expect(err).Should(Succeed(), "Failed to read p50 for all elasticsearch reads with status code 2xx")
		b.RecordValue("All elasticsearch 2xx reads p50", p50)

		// Record latency avg
		avg, err := metricsClient.RequestDurationOkSearchAvg(job, defaultRange)
		Expect(err).Should(Succeed(), "Failed to read average for all elasticsearch reads with status code 2xx")
		b.RecordValue("All elasticsearch 2xx reads avg", avg)
